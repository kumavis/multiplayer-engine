...
function StateMachine(stateTransFn) {
  ...
  function getFinalizedState(cb) {
    ...
    var tick = getFinalizedTick()
    getState(tick, cb)
  }

  function getOptimisticAction(tick){
    _debug('getOptimisticAction', tick)
    return optimisticActionHistory.get(tick) || {}
  }
  ...
  function getOptimisticActionHistory(){
    var firstTick = getFinalizedTick()
    var actionHistory = [] 
    for(var tick=firstTick; tick<=highestTick; tick++) {
      var action = getOptimisticAction(tick)
  ...
  function getAuthoritativeAction(tick){
    ...
    return authoritativeActionHistory.get(tick) || {}
  }
