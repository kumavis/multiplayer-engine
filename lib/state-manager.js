...
StateManager.prototype._setDirtyTick = function(newDirtyTick){
  ...
  if (lastGoodTick === null || newGoodTick<lastGoodTick) {
    // console.log('_setDirtyTick: _lastGoodTick =',newGoodTick)
...
StateManager.prototype.setState = function(newState, tick){
  ...
  // console.log('setState: _lastGoodTick =',tick, newState)
}

StateManager.prototype.getStateHistory = function(fromTick, toTick){
  // console.log('StateManager - getActionHistory(', fromTick, toTick)
  const self = this
  if (fromTick === undefined) fromTick = self.currentTick-self._historyLimit
  if (toTick === undefined) toTick = self.currentTick
  // lazy calculate state
  if (self.isDirty(toTick)) {
    self.run(toTick)
  }
  // copy range of history
  var stateHistory = self._stateHistory
  stateHistory.length = self.currentTick
  var historySlice = [].slice.call(stateHistory, fromTick, toTick)
  delete stateHistory.length
...
StateManager.prototype.getActionFrame = function(tick){
  ...
  const self = this
  if (tick === undefined) tick = self.currentTick
  var actionFrame = self._actionHistory[tick] || newActionFrame()
...
StateManager.prototype.getActionFrameHistory = function(fromTick, toTick){
  // console.log('StateManager - getActionHistory(', fromTick, toTick)
  const self = this
  if (fromTick === undefined) fromTick = self.currentTick-self._historyLimit
  if (toTick === undefined) toTick = self.currentTick
  // copy range of history
  var actionHistory = self._actionHistory
  actionHistory.length = self.currentTick
  var historySlice = [].slice.call(actionHistory, fromTick, toTick)
  delete actionHistory.length
